...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE PROCEDURE `sp0_<%=aux_table.Name%>_getObject_<%=aux_search.Name%>`(<%
...
for (int f = 0; f < aux_table.Fields.Count; f++) {
	...
	IN `<%=aux_field.Name%>` <%=aux_field.DBType_inDB_name%><%=(aux_field.isText) ? "(" + aux_field.Size + ")" : ""%> OUT<%=(f != aux_table.Fields.Count - 1) ? ", " : ""%><%
}%>
)
	NOT DETERMINISTIC
...
*/<%
	...
	for (int f = 0; f < aux_table.Fields.Count; f++) {
		aux_field = aux_table.Fields[f];%>
		t1.`<%=aux_field.Name%>` INTO `<%=aux_field.Name%>`, <%
	}%>
		true INTO `Exists`
	FROM `<%=aux_table.Name%>` t1
	INNER JOIN `fnc_<%=aux_table.Name%>_isObject_<%=aux_search.Name%>`(<%
	for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
		aux_field = aux_search.SearchParameters[f].Field;
		aux_field_name = aux_search.SearchParameters[f].ParamName;%>
