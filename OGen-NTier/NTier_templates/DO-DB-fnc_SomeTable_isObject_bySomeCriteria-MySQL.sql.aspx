...
for (int f = 0; f < _aux_search.SearchParameters.Count; f++) {
	...
	RETURNS `finalresult` TABLE (<%
		...
		`<%=_aux_field.Name%>` <%=_aux_field.DBs[_aux_dbervertype].DBType_inDB_name%><%=(_aux_field.isText) ? "(" + _aux_field.DBs[_aux_dbervertype].Size + ")" : ""%><%=(f != _aux_table.Fields_onlyPK.Count - 1) ? ", " : ""%><%
		}%>
	)
	NOT DETERMINISTIC
