...
%>CREATE PROCEDURE `sp_<%=_aux_table.Name%>_Record_open_<%=_aux_search.Name%>`(<%
	...
	IN `<%=_aux_field_name%>_search_` <%=_aux_field.DBs[_aux_dbervertype].DBType_inDB_name%><%=(_aux_field.isText) ? "(" + _aux_field.DBs[_aux_dbervertype].Size + ")" : ""%><%=(f != _aux_search.SearchParameters.Count - 1) ? ", " : ""%><%
	}%>
)
	NOT DETERMINISTIC
