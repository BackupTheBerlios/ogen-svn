...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE FUNCTION [dbo].[fnc_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>](<%
...
RETURN
	...
	for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
		...
		((
			...
			AND
			([<%=aux_field.Name%>] <%=(aux_field.isText) ? "LIKE '%' +" : "="%> @<%=aux_field_name%>_search_<%=(aux_field.isText) ? " + '%' COLLATE SQL_Latin1_General_CP1_CI_AI" : ""%>)
