...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE PROCEDURE `sp0_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>_fullmode`(<%
	for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
		//aux_field = er.Tables[aux_search.SearchParameters[f].TableIndex].Fields[aux_search.SearchParameters[f].FieldIndex];
		aux_field = aux_search.SearchParameters[f].Field;
		aux_field_name = aux_search.SearchParameters[f].ParamName;%>
	IN `<%=aux_field_name%>_search_` <%=aux_field.DBType_inDB_name%><%=(aux_field.isText) ? "(" + aux_field.Size + ")" : ""%><%=(f != aux_search.SearchParameters.Count - 1) ? ", " : ""%><%
	}%>
)
	NOT DETERMINISTIC
...
BEGIN
	...
	CALL `sp_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>`(<%
	for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
		aux_field = aux_search.SearchParameters[f].Field;
		aux_field_name = aux_search.SearchParameters[f].ParamName;
	...
	for (int f = 0; f < aux_table.Fields.Count; f++) {
		...
		t1.`<%=aux_field.Name%>`<%=(f != aux_table.Fields.Count - 1) ? ", " : ""%><%
	}%>
	...
	-- CHANGE where CONDITION IN: `fnc_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>`
	-- NOT HERE!

	-- CHANGE order by IN: `sp_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>`
	-- NOT HERE!
