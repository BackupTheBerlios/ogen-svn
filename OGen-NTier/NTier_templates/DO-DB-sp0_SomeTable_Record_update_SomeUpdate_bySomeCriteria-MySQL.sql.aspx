...
for (int f = 0; f < update.UpdateParameters.Count; f++) {
	...
	IN `<%=_aux_field.Name%>_update_` <%=_aux_field.DBs[_aux_dbervertype].DBType_inDB_name%><%=(_aux_field.isText) ? "(" + _aux_field.DBs[_aux_dbervertype].Size + ")" : ""%><%=(f != update.UpdateParameters.Count - 1) ? ", " : ""%><%
}%>
)
	NOT DETERMINISTIC
