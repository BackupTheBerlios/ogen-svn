...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE PROCEDURE `sp0_<%=aux_table.Name%>_Record_update_<%=update.Name%>_<%=aux_search.Name%>`(<%
...
for (int f = 0; f < update.UpdateParameters.Count; f++) {
	...
	IN `<%=aux_field.Name%>_update_` <%=aux_field.DBType_inDB_name%><%=(aux_field.isText) ? "(" + aux_field.Size + ")" : ""%><%=(f != update.UpdateParameters.Count - 1) ? ", " : ""%><%
}%>
)
	NOT DETERMINISTIC
