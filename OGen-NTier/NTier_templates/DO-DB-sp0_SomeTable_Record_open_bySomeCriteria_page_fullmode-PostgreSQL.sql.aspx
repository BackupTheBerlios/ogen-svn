...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE OR REPLACE FUNCTION "sp0_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>_page_fullmode"(<%
	...
	"<%=aux_field_name%>_search_" <%=aux_field.DBType_inDB_name%>, <%
	}%>
	"page_" Int,
	"page_numRecords_" Int
)
RETURNS SETOF "<%=aux_table.Name%>"
AS '
	DECLARE
		_Output "<%=aux_table.Name%>";
	BEGIN
		FOR _Output IN
			SELECT<%
			...
			INNER JOIN "sp_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>"(<%
			for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
				aux_field = aux_search.SearchParameters[f].Field;
				aux_field_name = aux_search.SearchParameters[f].ParamName;%>
