...
for (int f = 0; f < _aux_search.SearchParameters.Count; f++) {
	...
	"<%=_aux_field_name%>_search_" <%=_aux_field.DBs[_aux_dbervertype].DBType_inDB_name%>, <%
}
for (int f = 0; f < update.UpdateParameters.Count; f++) {
	_aux_field = update.UpdateParameters[f].Field;%>
	"<%=_aux_field.Name%>_update_" <%=_aux_field.DBs[_aux_dbervertype].DBType_inDB_name%><%=(f != update.UpdateParameters.Count - 1) ? ", " : ""%><%
}%>
)
RETURNS VOID AS '
	DECLARE
