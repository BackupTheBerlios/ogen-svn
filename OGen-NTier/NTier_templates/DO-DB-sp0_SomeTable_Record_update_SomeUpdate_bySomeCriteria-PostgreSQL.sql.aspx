...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE OR REPLACE FUNCTION "sp0_<%=aux_table.Name%>_Record_update_<%=update.Name%>_<%=aux_search.Name%>"(<%
...
for (int f = 0; f < update.UpdateParameters.Count; f++) {
	...
	"<%=aux_field.Name%>_update_" <%=aux_field.DBType_inDB_name%><%=(f != update.UpdateParameters.Count - 1) ? ", " : ""%><%
}%>
)
RETURNS VOID AS '
	DECLARE
