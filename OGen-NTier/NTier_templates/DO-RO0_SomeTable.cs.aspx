	...
	public sealed class RO0_<%=_aux_table.Name%> : RO__base {
		...
		public void Open_<%=_aux_table.Searches[s].Name%>(<%
			...
			IDbDataParameter[] _dataparameters = new IDbDataParameter[] {<%
				for (int f = 0; f < _aux_table.Searches[s].SearchParameters.Count; f++) {
					_aux_field = _aux_table.Searches[s].SearchParameters[f].Field;
					_aux_field_name = _aux_table.Searches[s].SearchParameters[f].ParamName;%><%=""%>
				parent_ref_.Connection.newDBDataParameter("<%=_aux_field_name%>_search_", DbType.<%=_aux_field.DBType_generic.Value.ToString()%>, ParameterDirection.Input, <%=_aux_field_name%>_search_in, <%=_aux_field.Size%>)<%=(f != _aux_table.Searches[s].SearchParameters.Count - 1) ? ", " : ""%><%
				}%>
			};
			base.Open(
				string.Format(
		...
		public void Open_<%=_aux_table.Searches[s].Name%>(<%
			...
			IDbDataParameter[] _dataparameters = new IDbDataParameter[] {<%
				for (int f = 0; f < _aux_table.Searches[s].SearchParameters.Count; f++) {
					_aux_field = _aux_table.Searches[s].SearchParameters[f].Field;
					_aux_field_name = _aux_table.Searches[s].SearchParameters[f].ParamName;%><%=""%>
				parent_ref_.Connection.newDBDataParameter("<%=_aux_field_name%>_search_", DbType.<%=_aux_field.DBType_generic.Value.ToString()%>, ParameterDirection.Input, <%=_aux_field_name%>_search_in, <%=_aux_field.Size%>), <%
				}%>
		...
		public void Update_<%=_aux_table.Searches[s].Updates[u].Name%>_<%=_aux_table.Searches[s].Name%>(<%
			...
			IDbDataParameter[] _dataparameters = new IDbDataParameter[] {<%
				...
				for (int f = 0; f < _aux_table.Searches[s].Updates[u].UpdateParameters.Count; f++) {
					_aux_field = _aux_table.Searches[s].Updates[u].UpdateParameters[f].Field;%>
				parent_ref_.Connection.newDBDataParameter("<%=_aux_field.Name%>_update_", DbType.<%=_aux_field.DBType_generic.Value.ToString()%>, ParameterDirection.Input, <%=_aux_field.Name%>_update_in, <%=_aux_field.Size%>)<%=(f != _aux_table.Searches[s].Updates[u].UpdateParameters.Count - 1) ? ", " : ""%><%
				}%>
			};
		...
		public bool hasObject_<%=_aux_table.Searches[s].Name%>(<%
			...
			for (int k = 0; k < _aux_table.Fields_onlyPK.Count; k++) {
				_aux_field = _aux_table.Fields_onlyPK[k];%><%=""%>
				parent_ref_.Connection.newDBDataParameter("<%=_aux_field.Name%>_", DbType.<%=_aux_field.DBType_generic.Value.ToString()%>, ParameterDirection.Input, <%=_aux_field.Name%>_in, <%=_aux_field.Size%>)<%=(k != _aux_table.Fields_onlyPK.Count - 1) ? ", " : ""%><%
			}
			for (int f = 0; f < _aux_table.Searches[s].SearchParameters.Count; f++) {
				_aux_field = _aux_table.Searches[s].SearchParameters[f].Field;
				_aux_field_name = _aux_table.Searches[s].SearchParameters[f].ParamName;
