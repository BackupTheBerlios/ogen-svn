...
int _aux_table_haspseudoidentitykey = _aux_table.hasPseudoIdentityKey();
...
%>CREATE PROCEDURE [dbo].[sp0_<%=_aux_table.Name%>_insObject]<%
	...
	@<%=_aux_field.Name%>_ <%=_aux_field.DBs[_aux_dbservertype].DBType_inDB_name%><%=(_aux_field.isText) ? " (" + _aux_field.Size + ")" : ""%><%=(_aux_field.isIdentity) ? " OUT" : ""%>, <%
	}%>
	@SelectIdentity_ Bit
AS<%
	...
	if (_aux_table_haspseudoidentitykey >= 0) {%>
		DECLARE @codSeed int SET @codSeed = 0
		EXEC [dbo].[sp_IdentitySeed__generator] 1234, @codSeed out<%
	}%>
		...
		INSERT INTO [<%=_aux_table.Name%>] (<%
			for (int f = 0; f < _aux_table.Fields.Count; f++) {
				_aux_field = _aux_table.Fields[f];
				if (
					_aux_field.isPseudoIdentity
					||
					(_aux_table_hasidentitykey != f)
				) {%>
			...
			for (int f = 0; f < _aux_table.Fields.Count; f++) {
				_aux_field = _aux_table.Fields[f];
				if (_aux_field.isPseudoIdentity) {%>
			@codSeed<%=(f != _aux_table.Fields.Count - 1) ? ", " : ""%><%
				} else if (_aux_table_hasidentitykey != f) {%>
		...
		if (_aux_table_haspseudoidentitykey >= 0) {%>
			SET @<%=_aux_table.Fields[_aux_table_hasidentitykey]%>_ = @codSeed<%
		} else {%>
