...
int aux_table_index = aux_metadata.Tables.Search(arg_TableName);
cDBMetadata_Table aux_table = aux_metadata.Tables[aux_table_index];
...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE OR REPLACE FUNCTION "fnc_<%=aux_table.Name%>_isObject_<%=aux_search.Name%>"(<%
...
RETURNS SETOF "v0_<%=aux_table.Name%>__onlyKeys" AS
'
	DECLARE
		_Output "v0_<%=aux_table.Name%>__onlyKeys";
	BEGIN
		FOR _Output IN
			SELECT<%
			...
			FROM "<%=aux_table.Name%>"<%=(makeItAComment) ? "/*" : ""%>
			WHERE<%
			for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
				aux_field = aux_search.SearchParameters[f].Field;
				aux_field_name = aux_search.SearchParameters[f].ParamName;%>
				("<%=aux_field.Name%>" <%=(aux_field.isText) ? "LIKE ''%'' ||" : "="%> "<%=aux_field_name%>_search_"<%=(aux_field.isText) ? " || ''%''" : ""%>)<%=(f != aux_search.SearchParameters.Count - 1) ? " AND" : ""%><%
			}%><%=(makeItAComment) ? "*/" : ""%>
		LOOP
			RETURN NEXT _Output;
		END LOOP;
