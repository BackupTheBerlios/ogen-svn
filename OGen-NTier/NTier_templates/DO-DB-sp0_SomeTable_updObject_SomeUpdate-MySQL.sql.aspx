...
for (int f = 0; f < aux_update.UpdateParameters.Count; f++) {
	...
	IN `<%=_aux_field.Name%>_update_` <%=_aux_field.DBs[_aux_dbervertype].DBType_inDB_name%><%=(_aux_field.isText) ? "(" + _aux_field.DBs[_aux_dbervertype].Size + ")" : ""%><%=(f != aux_update.UpdateParameters.Count - 1) ? ", " : ""%><%
}
