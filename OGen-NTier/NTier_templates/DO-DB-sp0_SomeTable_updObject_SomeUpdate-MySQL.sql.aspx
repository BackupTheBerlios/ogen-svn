...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE PROCEDURE `sp0_<%=aux_table.Name%>_updObject_<%=aux_update.Name%>`(<%
...
for (int f = 0; f < aux_update.UpdateParameters.Count; f++) {
	...
	IN `<%=aux_field.Name%>_update_` <%=aux_field.DBType_inDB_name%><%=(aux_field.isText) ? "(" + aux_field.Size + ")" : ""%><%=(f != aux_update.UpdateParameters.Count - 1) ? ", " : ""%><%
}
