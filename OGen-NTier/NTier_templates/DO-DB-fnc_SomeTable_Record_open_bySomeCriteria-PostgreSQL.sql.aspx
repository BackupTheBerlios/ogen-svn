...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE OR REPLACE FUNCTION "fnc_<%=aux_table.Name%>_Record_open_<%=aux_search.Name%>"(<%
...
RETURNS SETOF "v0_<%=aux_table.Name%>__onlyKeys"
AS '
	DECLARE
		...
		FOR _Output IN
			...
			for (int f = 0; f < aux_search.SearchParameters.Count; f++) {
				...
				((
					...
					AND
					("<%=aux_field.Name%>" <%=(aux_field.isText) ? "LIKE ''%'' ||" : "="%> "<%=aux_field.Name%>_search_"<%=(aux_field.isText) ? " || ''%'' /*COLLATE SQL_Latin1_General_CP1_CI_AI*/" : ""%>)
				))<%=(f != aux_search.SearchParameters.Count - 1) ? " AND" : ""%><%
                } else {%>
                ("<%=aux_field.Name%>" <%=(aux_field.isText) ? "LIKE ''%'' ||" : "="%> "<%=aux_field_name%>_search_"<%=(aux_field.isText) ? " || ''%'' /*COLLATE SQL_Latin1_General_CP1_CI_AI*/" : ""%>)<%=(f != aux_search.SearchParameters.Count - 1) ? " AND" : ""%><%
                }
